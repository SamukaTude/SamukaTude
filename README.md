# 👋 Olá! Eu sou Samuel Tude

💻 **Estudante de Programação** • 🎨 **UI/UX** • 🚀 **Em constante evolução**

Sou estudante do **3º ano do Ensino Médio** e do **Instituto Proa**, desenvolvendo meus conhecimentos em **programação, desenvolvimento web e design de interfaces**.

Busco transformar o que aprendo em **projetos práticos**, construir meu portfólio e evoluir continuamente como profissional da área de tecnologia.

---

## 🚀 Sobre mim

* 🎓 Estudante do **3º ano do Ensino Médio**
* 📚 Estudante do **Instituto Proa**
* 💻 Foco em **Programação e Desenvolvimento Web**
* 🧠 Conhecimentos em **Lógica de Programação**
* 🌐 Estudos em **HTML, CSS, JavaScript e Python**
* 🎨 **9 projetos desenvolvidos no Figma**
* 🖥️ Interesse em **UI/UX e desenvolvimento de interfaces**
* 🌱 Atualmente aprofundando meus conhecimentos em **JavaScript e Python**
* 🎯 Buscando minha **primeira oportunidade profissional em Tecnologia**

---

## 🛠️ Tecnologias & Ferramentas

### 💻 Desenvolvimento

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,python" />
</p>

### 🎨 UI/UX & Design

<p>
  <img src="https://skillicons.dev/icons?i=figma" />
</p>

### 🔧 Ferramentas

<p>
  <img src="https://skillicons.dev/icons?i=vscode,git,github" />
</p>

---

## 📂 Projetos

Aqui reúno projetos desenvolvidos durante meus estudos, colocando em prática os conhecimentos adquiridos.

### 🌐 Meu Primeiro Site

Projeto desenvolvido para praticar os fundamentos de **HTML e CSS**, trabalhando conceitos como:

* Estruturação de páginas
* Navegação entre páginas
* Imagens e links
* Listas e tabelas
* Classes e organização de elementos
* CSS e estilização
* Organização de arquivos

🔗 **Repositório:** Em desenvolvimento

---

### 🏦 Sistema Bancário

Projeto desenvolvido durante os estudos de **Lógica de Programação**, simulando operações básicas de um sistema bancário.

### Conceitos praticados:

* Variáveis
* Entrada e saída de dados
* Condições
* Estruturas de decisão
* Operações matemáticas
* Menus interativos
* Simulação de operações bancárias

🔗 **Repositório:** Em desenvolvimento

---

## 🎨 Projetos de UI/UX

Além da programação, também venho desenvolvendo conhecimentos em **UI/UX e design de interfaces**.

Atualmente possuo **9 projetos desenvolvidos no Figma**, utilizando diferentes conceitos de criação e organização visual.

### 💡 Competências praticadas

* 🎨 Design de interfaces
* 🧩 Prototipação
* 📱 Interfaces para diferentes dispositivos
* 🧠 Hierarquia visual
* 📐 Organização de layouts
* 🎯 Experiência do usuário
* 🖥️ Estruturação visual de páginas

🔗 **Portfólio Figma:** Em breve

---

## 📚 Atualmente estudando

| Tecnologia               | Foco                                |
| ------------------------ | ----------------------------------- |
| 🌐 HTML & CSS            | Estrutura e estilização de páginas  |
| ⚡ JavaScript             | Lógica e interatividade             |
| 🐍 Python                | Fundamentos e programação           |
| 🧠 Lógica de Programação | Estruturas e resolução de problemas |
| 🔧 Git & GitHub          | Versionamento e colaboração         |
| 🎨 UI/UX                 | Interfaces e experiência do usuário |

> Meu foco é construir uma base sólida, praticar através de projetos e transformar conhecimento em experiência.

---

## 🔥 Minha atividade

<p align="center">
  <img src="https://streak-stats.demolab.com?user=SamukaTude&theme=tokyonight" />
</p>

---

## 🎯 Próximos objetivos

* ⚡ Aprofundar meus conhecimentos em **JavaScript**
* 🐍 Evoluir em **Python**
* 🧠 Fortalecer minha **lógica de programação**
* 🔧 Aprimorar **Git e GitHub**
* 🎨 Desenvolver novos projetos de **UI/UX**
* 💻 Criar projetos próprios
* 🌐 Construir meu **portfólio profissional**
* 🚀 Desenvolver projetos cada vez mais completos
* 🎯 Conquistar minha **primeira oportunidade na área de tecnologia**

---

## 📫 Entre em contato

<a href="https://www.linkedin.com/in/samuel-tude-321684352/">
  <img src="https://img.shields.io/badge/LinkedIn-Perfil-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<p align="center">

### 🚀 Aprender • Praticar • Evoluir

**Obrigado por visitar meu perfil!**

</p>
